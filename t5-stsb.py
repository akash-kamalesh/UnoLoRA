...
def preprocess_mnli(examples):
    return {
        "input_text": [
            f"mnli: premise: {premise} hypothesis: {hypothesis}"
...
def preprocess_mnli_matched(examples):
    return {
        "input_text": [
            f"mnli_matched: premise: {premise} hypothesis: {hypothesis}"
            for premise, hypothesis in zip(examples["premise"], examples["hypothesis"])
        ],
        "target_text": [str(label) for label in examples["label"]],
...
def preprocess_mnli_mismatched(examples):
    return {
        "input_text": [
            f"mnli_mismatched: premise: {premise} hypothesis: {hypothesis}"
            for premise, hypothesis in zip(examples["premise"], examples["hypothesis"])
        ],
        "target_text": [str(label) for label in examples["label"]],
...
def preprocess_mrpc(examples):
    return {
        "input_text": [
            f"mrpc: sentence1: {s1} sentence2: {s2}"
...
def preprocess_qnli(examples):
    return {
        "input_text": [
            f"qnli: question: {question} sentence: {sentence}"
...
def preprocess_qqp(examples):
    return {
        "input_text": [
            f"qqp: question1: {q1} question2: {q2}"
...
def prepare_datasets():
    ...
    for task, preprocess_fn in datasets_info:
        ...
        if task == "mnli":
            ...
            validation_mismatched = load_dataset(
                "glue", task, split="validation_mismatched"
            )

            train_dataset = train_dataset.map(
                preprocess_mnli_mismatched,
                batched=True,
                batch_size=10000,
                num_proc=10,
                remove_columns=train_dataset.column_names,
            )
            validation_matched = validation_matched.map(
                preprocess_mnli_matched,
                batched=True,
                batch_size=10000,
                num_proc=10,
                remove_columns=validation_matched.column_names,
            )
            validation_mismatched = validation_mismatched.map(
                preprocess_mnli_mismatched,
                batched=True,
                batch_size=10000,
                num_proc=10,
                remove_columns=validation_mismatched.column_names,
            )
            ...
            if len(train_dataset) > 10000 or task == "stsb":
                train_indices, new_validation_indices = get_train_validation_split(
                    train_dataset
                )
            ...
            #     test_dataset = load_dataset("glue", task, split="test")
                ...
                validation_indices, test_indices = get_validation_test_split(
                    validation_dataset
                )
...
def tokenize_function(examples):
    ...
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(
            targets, padding="max_length", truncation=True, max_length=64
        )
...
def prepare_full_data_pipeline():
    ...
    validation_dataset = combined_validation_dataset.map(
        tokenize_function,
        batched=True,
        num_proc=15,
        batch_size=40000,
    ...
    train_dataset.set_format(
        type="torch", columns=["input_ids", "attention_mask", "labels", "task_type"]
    )
    ...
    logger.info(
        "Data preprocessing with task relationship analysis and visualization completed successfully."
    )
...
def prepare_dataloaders(
    train_dataset, validation_dataset, test_dataset, batch_size, num_workers=4
):
    # Calculate distribution
    distribution = calculate_dataset_distribution(
    ...
    print(
        f"{'Task Name':<20} {'Train Samples':<15} {'Val Samples':<15} {'Test Samples':<15} {'Total Samples':<15}"
    )
    print("-" * 80)
    ...
    for task, counts in distribution.items():
        ...
        print(
            f"{task:<20} {train_count:<15} {val_count:<15} {test_count:<15} {total_count:<15}"
        )
    ...
    print("-" * 80)
    print(
        f"{'Total':<20} {total_train:<15} {total_val:<15} {total_test:<15} {total_train + total_val + total_test:<15}"
    )
...
wandb.init(
    ...
    name="t5-separate-lora-stsb-seed42_t5-base",
...
def evaluate_task(model, dataset, task_type, metric_name, task_to_id):
    ...
    dataloader = DataLoader(
        dataset, batch_size=32, collate_fn=lambda batch: collate_fn(batch, task_to_id)
    )
    ...
    if task_type == "cola":
        ...
        # Convert predictions and labels to float
        ...
        try:
            if task == "mnli":
                ...
                results_table.append(
                    [f"{task} matched", f"{matched_results[main_metric]:.4f}"]
                )
                results_table.append(
                    [f"{task} mismatched", f"{mismatched_results[main_metric]:.4f}"]
                )
                wandb.log(
                    {
                        f"final_{main_metric}_mnli_matched": matched_results[
                            main_metric
                        ],
                        f"final_{main_metric}_mnli_mismatched": mismatched_results[
                            main_metric
                        ],
                    }
                )
                ...
                results_table.append(
                    [
                        task,
                        f"Pearson: {results['pearson']:.4f}, Spearman: {results['spearman']:.4f}",
                    ]
                )
                wandb.log(
                    {
...
class CustomTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        task_metrics = defaultdict(
            lambda: {"predictions": [], "labels": [], "samples": 0}
        )
        ...
        for step, inputs in enumerate(dataloader):
            ...
            for task, pred, label in zip(
                batch_task_types, decoded_preds, decoded_labels
            ):
        ...
        overall_accuracy = sum(
            metrics.get(f"{task}_accuracy", 0.0) * metrics.get(f"{task}_samples", 0)
            for task in classification_tasks
        )
        overall_samples = sum(
            metrics.get(f"{task}_samples", 0) for task in classification_tasks
        )
        metrics["overall_accuracy"] = (
            overall_accuracy / overall_samples if overall_samples > 0 else 0.0
        )
        ...
        # Return metrics with the "eval_" prefix for compatibility with Trainer
        ...
        return EvalLoopOutput(
            ...
            num_samples=sum(
                task_data["samples"] for task_data in task_metrics.values()
            ),
        )
...
def collate_fn(
    batch: list, task_to_id: Dict[str, int]
) -> Optional[Dict[str, Union[torch.Tensor, Any]]]:
    valid_examples = [
        x
        for x in batch
        if all(
            key in x for key in ["input_ids", "attention_mask", "labels", "task_type"]
        )
    ]
    ...
    attention_mask = torch.stack(
        [torch.tensor(x["attention_mask"]) for x in valid_examples]
    )
...
training_args = TrainingArguments(
    ...
    metric_for_best_model="stsb_corr",
...
trainer.train()
call()
trainer.save_model("shared_stsb_seed42_t5-base")
